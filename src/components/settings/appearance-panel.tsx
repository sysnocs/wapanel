...
export function AppearancePanel() {
  const { theme, setTheme, mode, setMode, appTitle, setAppTitle } = useTheme();
  const { account, canEditSettings } = useAuth();
  const t = useTranslations("Settings.appearance");
  const [titleDraft, setTitleDraft] = useState(account?.app_title ?? appTitle);

  const saveTitle = () => {
    const normalized = titleDraft.trim();
    if (!normalized || normalized.length > 60 || !canEditSettings) return;
    void setAppTitle(normalized).catch(() => {
      toast.error(t("webTitleSaveFailed"));
    });
  };
  ...
  return (
    <section className="max-w-3xl animate-in fade-in-50 duration-200">
      ...
      <div className="mb-8 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">{t("webTitleDescription")}</p>
        <div className="flex max-w-xl flex-col gap-2 sm:flex-row sm:items-end">
          <div className="flex-1 space-y-1.5">
            <Label htmlFor="web-title">{t("webTitleLabel")}</Label>
            <Input
              id="web-title"
              value={titleDraft}
              maxLength={60}
          ...
          <Button type="button" onClick={saveTitle} disabled={!canEditSettings}>
            {t("saveWebTitle")}
          </Button>
        </div>
      </div>
