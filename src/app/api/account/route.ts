...
export async function PATCH(request: Request) {
  try {
    ...
    const body = (await request.json().catch(() => null)) as
      | { name?: unknown; app_title?: unknown }
      | null;
    const rawName = body?.name;
    const rawAppTitle = body?.app_title;

    if (rawAppTitle !== undefined) {
      if (typeof rawAppTitle !== "string") {
        return NextResponse.json(
          { error: "'app_title' must be a string" },
          { status: 400 },
        );
      }
      const appTitle = rawAppTitle.trim();
      if (appTitle.length === 0 || appTitle.length > MAX_APP_TITLE_LEN) {
        return NextResponse.json(
          { error: `Web title must be between 1 and ${MAX_APP_TITLE_LEN} characters` },
          { status: 400 },
        );
      }
      ...
      const { data, error } = await ctx.supabase
        ...
        .eq("id", ctx.accountId)
        .select("id, name, app_title")
        .single();

      if (error) {
        console.error("[PATCH /api/account] app title update error:", error);
        return NextResponse.json(
          { error: "Failed to update web title" },
          { status: 500 },
        );
      }
