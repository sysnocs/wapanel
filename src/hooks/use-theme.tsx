...
interface ThemeContextValue {
  ...
  toggleMode: () => void;
  appTitle: string;
  applyAppTitle: (next: string) => void;
  setAppTitle: (next: string) => Promise<void>;
}
...
function readInitialAppTitle(): string {
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [appTitle, setAppTitleState] = useState<string>(readInitialAppTitle);
  ...
  const applyAppTitle = useCallback((next: string) => {
    const normalized = next.trim().slice(0, 60) || DEFAULT_APP_TITLE;
    setAppTitleState(normalized);
    document.title = normalized;
  }, []);

  const setAppTitle = useCallback(async (next: string) => {
    const normalized = next.trim().slice(0, 60) || DEFAULT_APP_TITLE;
    const response = await fetch("/api/account", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ app_title: normalized }),
    });
    if (!response.ok) {
      throw new Error("Failed to save web title");
    }
    applyAppTitle(normalized);
  }, [applyAppTitle]);
  ...
  }, [theme, mode, appTitle, applyAppTitle]);
