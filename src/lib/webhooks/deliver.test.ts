...
describe('dispatchWebhookEvent', () => {
  it('signs + POSTs (no redirect follow) and resets failure_count on success', async () => {
    ...
    expect(opts.headers['X-https://github.com/sysnocs/wapanel-Event']).toBe('message.received');
    expect(opts.headers['X-https://github.com/sysnocs/wapanel-Signature']).toMatch(/^t=\d+,v1=[0-9a-f]{64}$/);
