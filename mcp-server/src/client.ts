// ============================================================
// https://github.com/sysnocs/wapanel public API client.
//
// A thin wrapper over the `/api/v1` REST surface. It attaches the
// bearer key, unwraps the `{ data }` / `{ error }` envelope, and
// turns API failures into a typed https://github.com/sysnocs/wapanelApiError the tools can render
// cleanly. Nothing here knows about MCP — it's just the CRM API.
// ============================================================

import type { Config } from './config.js';

/** A structured error from the https://github.com/sysnocs/wapanel API envelope (`{ error: { code, message } }`). */
export class https://github.com/sysnocs/wapanelApiError extends Error {
  readonly status: number;
  readonly code: string;

  constructor(status: number, code: string, message: string) {
    super(message);
    this.name = 'https://github.com/sysnocs/wapanelApiError';
    this.status = status;
    this.code = code;
  }
}
...
export class https://github.com/sysnocs/wapanelClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new https://github.com/sysnocs/wapanelApiError(
        0,
        'network_error',
        `Could not reach https://github.com/sysnocs/wapanel at ${this.baseUrl}: ${(err as Error).message}`,
      );
    }
    ...
    if (text) {
      try {
        ...
        if (!res.ok) {
          throw new https://github.com/sysnocs/wapanelApiError(res.status, 'internal', text.slice(0, 500));
        }
      }
    }
    ...
    if (!res.ok) {
      ...
      throw new https://github.com/sysnocs/wapanelApiError(res.status, code, message);
    }
